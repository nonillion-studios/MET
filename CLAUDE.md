...
- **Layer masks/smart objects** (layer *groups* and stack-aware adjustment layers *are* implemented — see Per-page layer stack above), **magnetic lasso**, **patch tool**, **slice tool**, **healing brush** (spot-heal *is* real), **curvature pen / path selection / direct selection** — declared as intended features (in tool lists, type unions, or SPEC) but not implemented.
- **Selecting a text or adjustment layer navigates the dock away from the Layers panel.** `selectLayer` calls `dock.selectTab('text'|'adjustment')`, and all three share the `top` region (`dockLayout.ts`), so the Layers panel unmounts on the very click that expands the row. `expandedLayerId` is lifted to `Studio` so the row survives the round trip and you can click back to Layers to reach its opacity — but it still takes two clicks, and it's why the auto-switch is worth revisiting. Each is a genuinely separate chunk of work. (Selection feather/expand/contract/add-subtract, liquify's `reconstruct` mode, Crop, TypeR's Multi-Bubble mode, and brush symmetry *are* implemented — see Canvas engine / TypeR above.)
